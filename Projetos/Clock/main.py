...
root = tk.Tk()
root.title("Relógio Análogico")
canvas = tk.Canvas(root, width=WIDTH, height=HEIGHT, bg="white")  # Configurações simples
canvas.pack()
...
def update_clock():
    ...
    # Desenhando a face do relógio
    canvas.create_oval(2, 2, WIDTH, HEIGHT, outline="black", width=2)

    # Desenhando os números das horas
    for i in range(12):
    ...
    # Desenhando a linha dos minutos
    for i in range(60):
    ...
    canvas.create_line(WIDTH/2, HEIGHT/2, minute_x, minute_y, fill="red", width=2)


canvas.after(2000, update_clock)
